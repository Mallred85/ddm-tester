...
export default function QuoteForm({ endpoint, showPlanSelector = false, defaultNeighborhood = '', defaultServices = [] }: Props) {
  ...
  if (status === 'success') {
    return (
      <div className="bg-white rounded-[2rem] border border-green-200 p-10 text-center shadow-2xl shadow-green-900/10">
  ...
  return (
    <form
      ...
      className="bg-white/95 backdrop-blur-xl rounded-[2rem] border border-green-100 shadow-2xl shadow-green-900/10 p-8 sm:p-10 space-y-6"
      noValidate
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            onChange={set('name')}
            required
            className="w-full h-12 px-4 rounded-xl border border-gray-200 bg-white text-gray-950 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent text-sm font-semibold transition"
          />
        </div>
        <div>
          <label className="block text-sm font-semibold text-gray-900 mb-1.5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select
            ...
            required
            className="w-full h-12 px-4 rounded-xl border border-gray-200 bg-white text-gray-950 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent text-sm font-semibold transition quote-area-select"
          >
            <option value="" className="bg-white text-gray-600">Select area...</option>
            {NEIGHBORHOODS.map((n) => (
              <option key={n} value={n} className="bg-white text-gray-950">{n}, UT</option>
            ))}
          </select>
        </div>
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          rows={3}
          className="w-full px-4 py-3 rounded-xl border border-gray-200 bg-white text-gray-950 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent text-sm font-semibold resize-none transition"
        />
      </div>
      ...
      <button
        ...
        className="w-full py-4 bg-gradient-to-r from-green-600 to-green-800 text-white font-black rounded-2xl hover:from-green-500 hover:to-green-700 transition-all shadow-xl shadow-green-700/25 hover:scale-[1.01] text-base flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
      >
        {status === 'loading' ? (
          <>
      ...
      <p className="text-center text-xs font-medium text-gray-600">
        We'll reach out to{' '}
        <span className="font-bold text-gray-900">{form.email || 'your email'}</span>{' '}
